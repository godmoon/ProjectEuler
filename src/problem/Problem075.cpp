...
namespace GET_NAMESPACE_NAME(PROBLEM_NO)
{
    ...
    // 获得线长l，能形成多少个整数边直角三角形
    uint32_t GetRightAngleTriangleCount(uint32_t l)
    {
        uint32_t count = 0;
        uint32_t halfL = l / 2;
        ...
        // a^2 + b^2 = c^2
        // a + b + c = l
        for (uint32_t a = 1; a < halfL; a++)
        {
            for (uint32_t b = a; a + b < l; ++b)
            {
                uint32_t c = l - a - b;
...
string GET_CLASS_NAME(PROBLEM_NO)::Run()
{
    ...
    for (uint32_t i = 0; i < THREAD_COUNT; ++i)
    {
        thread th([&](uint32_t threadId)
        {
            ...
            while ((n = currN.fetch_add(1)) <= COUNT)
            {
                ...
                uint32_t count = GetRightAngleTriangleCount(n);
                // cout << n << ":" << count << endl;
                if (count == 1)
                {
                    ++resultSum;
    ...
    printf("线长小于[%u]一共能组成[%lu]个整数直角三角形.\n", COUNT, resultSum.load());
