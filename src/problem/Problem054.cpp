...
namespace GET_NAMESPACE_NAME(PROBLEM_NO)
{
    ...
    public:
        ...
        // Returns:  
        //************************************  
        Card(const string &cards, uint32_t index);
        ...
        // Returns:   uint32_t  
        //************************************  
        static uint32_t GetNum(char numCh);
    ...
    Card::Card(const string &cards, uint32_t index)
    {
    ...
    uint32_t Card::GetNum(char numCh)
    {
        const static string CH_TO_NUM = "  23456789TJQKA";

        for (int32_t i = CH_TO_NUM.length() - 1; i >= 2; --i)
        {
    ...
    unsigned char Card::GetSuit(unsigned char suitCh)
    {
        ...
        for (int32_t i = CH_TO_SUIT.length() - 1; i >= 0; --i)
        {
    ...
    {
    public:
        const static uint32_t CARD_NUM = 5;                   // 手牌数目  
        static const uint32_t MIN_LENGTH = 3 * CARD_NUM - 1;  // 一手牌最小长度  
        ...
        // Returns:   bool  
        //************************************  
        bool IsHighCard(vector<Card> &cardPara)const;

        //************************************  
        // Method:    IsOnePair  
        // Access:    public  
        // Describe:  找到最大一对数值相同的  
        // Parameter: vector<Card> & cardPara  
        // Returns:   bool  
        //************************************  
        bool IsOnePair(vector<Card> &cardPara)const;
        ...
        // Returns:   bool  
        //************************************  
        bool IsTwoPairs(vector<Card> &cardPara)const;
        ...
        // Returns:   bool  
        //************************************  
        bool IsThreeOfAKind(vector<Card> &cardPara)const;

        //************************************  
        // Method:    IsStraight  
        // Access:    public  
        // Describe:  所有牌是否都是按顺序排列的  
        // Parameter: vector<Card> & cardPara   记录数值最大的一张牌  
        // Returns:   bool  
        //************************************  
        bool IsStraight(vector<Card> &cardPara)const;

        //************************************  
        // Method:    IsFlush  
        // Access:    public  
        // Describe:  所有牌是否都是同花色  
        // Parameter: vector<Card> & cardPara   记录面值最大的一张牌  
        // Returns:   bool  
        //************************************  
        bool IsFlush(vector<Card> &cardPara)const;

        //************************************  
        // Method:    IsFullHouse  
        // Access:    public  
        // Describe:  是否是2张和3张的组合  
        // Parameter: vector<Card> & cardPara   记录一张3张同值牌和一张2张同值牌，3张的牌放在索引0的位置  
        // Returns:   bool  
        //************************************  
        bool IsFullHouse(vector<Card> &cardPara)const;

        //************************************  
        // Method:    IsFourOfAKind  
        // Access:    public  
        // Describe:  是否有4张相同的牌  
        // Parameter: vector<Card> & cardPara   记录4张牌中花色最大的一张  
        // Returns:   bool  
        //************************************  
        bool IsFourOfAKind(vector<Card> &cardPara)const;
        ...
        // Returns:   bool  
        //************************************  
        bool IsStraightFlush(vector<Card> &cardPara)const;

        //************************************  
        // Method:    IsRoyalFlush  
        // Access:    public  
        // Describe:  是否是最大同花顺（面值为：T,J,Q,K,A）  
        // Parameter: vector<Card> & cardPara   记录A  
        // Returns:   bool  
        //************************************  
        bool IsRoyalFlush(vector<Card> &cardPara)const;
        ...
        // Access:    public  
        // Describe:  获取此手牌的组合类型  
        // Parameter: vector<Card> & cardPara   组合类型参数，用于同类型比较大小  
        // Returns:   Hand::HandType  
        //************************************  
        HandType GetHandType(vector<Card> &cardPara)const;
    ...
    private:
        ...
        // Access:    private  
        // Describe:  手牌类型函数指针  
        // Parameter: vector<Card> &cardPara    相关联的牌  
        //************************************  
        typedef bool (Hand::*TYPE_FUN)(vector<Card> &cardPara) const;
    ...
    bool Hand::IsHighCard(vector<Card> &cardPara)const
    {
        cardPara.clear();
    ...
    bool Hand::IsOnePair(vector<Card> &cardPara) const
    {
        // 从大往小寻找  
        for (int32_t i = CARD_NUM - 1; i >= 1; --i)
        {
            // 有一对数值相同  
    ...
    bool Hand::IsTwoPairs(vector<Card> &cardPara) const
    {
    ...
    bool Hand::IsThreeOfAKind(vector<Card> &cardPara) const
    {
        // 从大往小寻找  
        for (int32_t i = CARD_NUM - 1; i >= 2; --i)
        {
            // 有3张数值相同  
    ...
    bool Hand::IsStraight(vector<Card> &cardPara) const
    {
        // 从大往小寻找  
        for (int32_t i = CARD_NUM - 1; i >= 1; --i)
        {
    ...
    bool Hand::IsFlush(vector<Card> &cardPara) const
    {
        // 从小往大寻找  
        for (uint32_t i = 1; i < CARD_NUM; ++i)
        {
            // 花色不同  
    ...
    bool Hand::IsFullHouse(vector<Card> &cardPara) const
    {
    ...
    bool Hand::IsFourOfAKind(vector<Card> &cardPara) const
    {
        uint32_t sameCount = 0;   // 相同的牌的张数  
    ...
    bool Hand::IsStraightFlush(vector<Card> &cardPara) const
    {
        return IsFlush(cardPara) && IsStraight(cardPara);
    }

    bool Hand::IsRoyalFlush(vector<Card> &cardPara) const
    {
    ...
    Hand::HandType Hand::GetHandType(vector<Card> &cardPara) const
    {
        for (int32_t handType = MAX_HAND_TYPE - 1; handType >= 0; --handType)
        {
    ...
    {
        vector<Card> thisHandCard;
        vector<Card> rightHandCard;
        ...
        {
            // 比较参数  
            uint32_t handCardSize = thisHandCard.size();
            for (uint32_t handCardIndex = 0; handCardIndex < handCardSize; ++handCardIndex)
            {
            ...
            // 参数相同，从大到小比较牌数值  
            for (int32_t cardIndex = CARD_NUM - 1; cardIndex >= 0; --cardIndex)
            {
...
string GET_CLASS_NAME(PROBLEM_NO)::Run()
{
    try
    {
        uint32_t player1WinCount = 0;
        uint32_t size = ARRAY_SIZE(GAME_CARDS);
        ...
        return CppString::ToString(player1WinCount);
    }
...
/*
[2014-07-15 13:04:02.862761]开始运行第54题
玩家1胜利的次数为376
[2014-07-15 13:04:02.866147]第54题运行完毕: From start 3386 us,from last 3386 us
By Moon
*/